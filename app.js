...
function getCatalogKeyFromUrl(url) {
  const cleanUrl = String(url ?? "").trim();
  const match = cleanUrl.match(/\/([^\/?#]+)\.(?:jpe?g|png|webp)(?:[?#].*)?$/i);
  return match ? match[1].toUpperCase() : "";
}

function collectUploadUrls(value) {
  if (!value) return [];
  ...
  if (Array.isArray(value)) return value.flatMap(collectUploadUrls);
  if (typeof value === "object") return Object.values(value).flatMap(collectUploadUrls);
  return [];
}

function setUploadUrls(data) {
  uploadUrlByCatalog = {};
  collectUploadUrls(data).forEach(url => {
    const catalogKey = getCatalogKeyFromUrl(url);
    if (catalogKey) uploadUrlByCatalog[catalogKey] = url;
  });
}

function getUploadImageSrc(catalog, triedUrls = []) {
  const catalogKey = String(catalog ?? "").toUpperCase();
  const tried = new Set(triedUrls.map(url => String(url ?? "")));

  if (uploadUrlByCatalog[catalogKey] && !tried.has(uploadUrlByCatalog[catalogKey])) {
    return uploadUrlByCatalog[catalogKey];
  }

  const fallbackKey = Object.keys(uploadUrlByCatalog)
    .find(key => key.startsWith(catalogKey) && !tried.has(uploadUrlByCatalog[key]));
  return fallbackKey ? uploadUrlByCatalog[fallbackKey] : "";
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  Promise.all([
    fetch(API + '?action=inventory').then(res => res.json()),
    db.collection("uploadedUrls").doc("urls").get().catch(() => null)
  ])
    .then(([data, uploadUrlsDoc]) => {
      if (uploadUrlsDoc?.exists) setUploadUrls(uploadUrlsDoc.data());
      console.log("Fetched Inventory:", uploadUrlByCatalog);
      renderProducts(data);
      hideLoading();
    });
...
function productCard(p) {
  const initialSrc = getUploadImageSrc(p.catalog) || `images/${p.catalog}.jpg`;
...
function handleLocalFallback(img, catalog) {
  ...
  const triedUrls = (img.dataset.triedUrls || "")
    .split("|")
    .filter(Boolean);
  const nextUploadSrc = getUploadImageSrc(catalog, triedUrls);

  if (nextUploadSrc) {
    img.dataset.triedUrls = [...triedUrls, nextUploadSrc].join("|");
    img.src = nextUploadSrc;
    return;
  }
